...
def test_socketio_connection():
    ...
    log.info("Step 1: HTTP handshake...")
    ...
    log.info(f"  Response: {handshake_data}")
    ...
    if handshake_data.startswith('0'):
        ...
        log.error(f"  ERROR: Unexpected handshake format")
        return
    conn.close()
    ...
    if '101 Switching Protocols' in response_str:
        ...
        log.error(f"  ✗ WebSocket upgrade failed: {response_str[:200]}")
        sock.close()
        return
    ...
    if header_end >= 0:
        ...
        if remaining:
            log.info(f"  Data after headers: {remaining.hex()}")
            try:
    ...
    log.info(f"  Sent: '40' ({len(frame)} bytes, frame hex: {frame.hex()})")
    ...
    log.info(f"  Sent: '2' ({len(ping_frame)} bytes)")
            ...
            if opcode == 1:  # Text frame
                try:
                    ...
                    if message.startswith('2'):
                        log.info(f"  → Engine.IO ping received, sending pong...")
                        ...
                        log.info(f"  ✓ Sent Engine.IO pong")
                except UnicodeDecodeError:
                    log.error(f"  ✗ Failed to decode: {payload_data.hex()[:50]}")
            elif opcode == 9:  # Ping frame (WebSocket level)
                log.info(f"  → WebSocket ping received, sending pong...")
                pong_mask = bytes(random.getrandbits(8) for _ in range(4))
                ...
                sock.send(bytes(pong_frame))
                log.info(f"  ✓ Sent WebSocket pong")
            elif opcode == 8:  # Close frame
                log.error(f"  ✗ Close frame received")
                break
            else:
                log.info(f"  ? Unknown opcode: {opcode}")
        except socket.timeout:
            log.info(f"  Timeout waiting for messages")
            break
    ...
    log.info(f"\n=== Summary ===")
    log.info(f"Total messages received: {len(received_messages)}")
    for msg_type, msg_content in received_messages:
        log.info(f"  - {msg_type}: {msg_content}")
